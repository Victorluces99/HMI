...
@NodeFactory.Registration(projectType = "com-prueba-hmipanelsubprojectcategory", position = 80)
public class HMICategoryWarningManagementNodeFactory implements NodeFactory{
@Override
    public NodeList<?> createNodes(Project project) {
        ...
        return new HMICategoryWarningManagementNodeList(projectDir);
    }

    private static class HMICategoryWarningManagementNodeList implements NodeList<FileObject>, FileChangeListener {

        private final FileObject folder;
        private final List<ChangeListener> listeners = new ArrayList<>();
        private final List<FileObject> keys = new ArrayList<>();
        private final static String HIDE_FILE_WMG = "base.wmg";

        public HMICategoryWarningManagementNodeList(FileObject folder) {
            this.folder = folder;
            refreshKeys();
        }
        // 1. Devuelve la lista de llaves (archivos .wmg) actual
        @Override
        ...
        public Node node(FileObject key) {
            try {
                ...
                return new HMIWarningManagementNode(dataObj, key);
        ...
        private void refreshKeys() {
            ...
            for (FileObject child : folder.getChildren()) {
                if (!child.isFolder() && "wmg".equalsIgnoreCase(child.getExt())) {

                    if (!HIDE_FILE_WMG.equalsIgnoreCase(child.getNameExt())) {
                        keys.add(child);
                    }
        ...
        public void fileDataCreated(FileEvent fe) {
            if ("wmg".equalsIgnoreCase(fe.getFile().getExt())) {
                refreshKeys();
            }
        }
    ...
    private static class HMIWarningManagementNode extends DataNode {

        private final FileObject fileObject;

        public HMIWarningManagementNode(DataObject dataObject, FileObject fileObject) {
        ...
        public Action[] getActions(boolean context) {
            ...
            allActions.add(new HMICategoryWarningManagementAction(fileObject));
            allActions.add(null);
