...
public class HMICategoryRuntimeUserManagementNodeFactory implements NodeFactory {
    ...
    public NodeList<?> createNodes(Project project) {
        ...
        return new HMICategoryRuntimeUserManagementNodeList(projectDir);
    }

    private static class HMICategoryRuntimeUserManagementNodeList implements NodeList<FileObject>, FileChangeListener {

        private final FileObject folder;
        private final List<ChangeListener> listeners = new ArrayList<>();
        private final List<FileObject> keys = new ArrayList<>();
        private final static String HIDE_FILE_RTUM = "base.rtum";

        public HMICategoryRuntimeUserManagementNodeList(FileObject folder) {
            this.folder = folder;
            refreshKeys();
        }
        // 1. Devuelve la lista de llaves (archivos .rtum) actual
        @Override
        ...
        public Node node(FileObject key) {
            try {
                ...
                return new HMIRuntimeUserManagementNode(dataObj, key);
        ...
        private void refreshKeys() {
            ...
            for (FileObject child : folder.getChildren()) {
                if (!child.isFolder() && "rtum".equalsIgnoreCase(child.getExt())) {

                    if (!HIDE_FILE_RTUM.equalsIgnoreCase(child.getNameExt())) {
                        keys.add(child);
                    }
        ...
        public void fileDataCreated(FileEvent fe) {
            if ("rtum".equalsIgnoreCase(fe.getFile().getExt())) {
                refreshKeys();
            }
        }
    ...
    private static class HMIRuntimeUserManagementNode extends DataNode {

        private final FileObject fileObject;

        public HMIRuntimeUserManagementNode(DataObject dataObject, FileObject fileObject) {
        ...
        public Action[] getActions(boolean context) {
            ...
            allActions.add(new HMICategoryRuntimeUserManagementAction(fileObject));
            allActions.add(null);
